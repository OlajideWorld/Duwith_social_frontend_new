...
showNewsComments({required BuildContext context, required String postId}) {
  return showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      builder: (context) {
        return CommentsDisplayNewsBottomSheet(
          postId: postId,
        );
      });
}

class CommentsDisplayNewsBottomSheet extends StatefulWidget {
  final String postId;
  const CommentsDisplayNewsBottomSheet({
    super.key,
    required this.postId,
  });

  @override
  State<CommentsDisplayNewsBottomSheet> createState() =>
      _CommentsDisplayNewsBottomSheetState();
}

class _CommentsDisplayNewsBottomSheetState
    extends State<CommentsDisplayNewsBottomSheet> {
            ...
            SizedBox(
              child: Column(
                ...
                children: [
                  ...
                  CText(
                    text: "${homeController.commentsNews.length} Comments",
                    size: 18,
                  ...
                  homeController.loadingComment.value == true
                      ...
                      : homeController.commentsNews.isEmpty
                          ? const SizedBox()
                          : Expanded(
                              child: ListView.builder(
                                  itemCount: homeController.commentsNews.length,
                                  itemBuilder: (context, index) {
                                    return commentsListView(
                                        context,
                                        maxWidth,
                                        homeController.commentsNews.value,
                                        homeController.commentsNews
                                            .value[index], () async {
                                      await socketService.likeNewsComment(
                                          homeController
                                              .commentsNews.value[index].id,
                              ...
                              onTap: () async {
                                if (homeController.commentsText.text.trim() ==
                                  ...
                                  final data = {
                                    ...
                                    "parentComment": homeController
                                            .isReply.value
                                        ? homeController.parentCommentId.value
                                        : null,
                                  };
                                  await socketService.addNewsComment(data);
                                  await socketService
                                      .getCommentByNewsId(widget.postId);
